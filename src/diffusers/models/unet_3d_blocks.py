...
class AttnDownBlock3D(nn.Module):
    def __init__(
        ...
        if num_attention_heads is None:
            logger.warn(
                f"It is not recommend to pass `num_attention_heads=None`. Defaulting `num_attention_heads` to `in_channels`: {out_channels}."
            )
            num_attention_heads = out_channels
        ...
        for i in range(num_layers):
            ...
            attentions.append(
                Attention(
                    out_channels,
                    heads=out_channels // num_attention_heads,
                    dim_head=num_attention_heads,
...
class UNetMidBlock3D(nn.Module):
    ...
    def __init__(
        ...
        if num_attention_heads is None:
            logger.warn(
                f"It is not recommend to pass `num_attention_heads=None`. Defaulting `num_attention_heads` to `in_channels`: {in_channels}."
            )
            num_attention_heads = in_channels
        ...
        for _ in range(num_layers):
            if self.add_attention:
                attentions.append(
                    Attention(
                        in_channels,
                        heads=in_channels // num_attention_heads,
                        dim_head=num_attention_heads,
                        rescale_output_factor=output_scale_factor,
                        eps=resnet_eps,
    ...
    def forward(self, hidden_states: torch.FloatTensor, temb: Optional[torch.FloatTensor] = None, num_frames: int = 1) -> torch.FloatTensor:
        hidden_states = self.resnets[0](hidden_states, temb)
        hidden_states = self.temp_convs[0](hidden_states, num_frames=num_frames)
